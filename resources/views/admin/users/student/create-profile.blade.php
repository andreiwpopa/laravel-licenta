<x-admin-layout>
    ...
    <div class="w-full sm:max-w-md mx-auto mt-6 px-6 py-4 bg-white dark:bg-gray-800 shadow-md overflow-hidden sm:rounded-lg">
        <form method="POST" action="{{ route('admin.users.store-student-profile') }}">
            @csrf

            <input type="hidden" name="id" value="{{ $uid }}">

            <div>
                <label for="nume_dupa_casatorie" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Nume dupa casatorie</label>
                <input type="text" id="nume_dupa_casatorie" name="nume_dupa_casatorie" class="mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
            </div>
